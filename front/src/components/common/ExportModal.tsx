...
import { FileText, Table, Info, X, CheckCircle2, Code } from "lucide-react";
...
export default function ExportModal({ isOpen, onClose }: ExportModalProps) {
  const formats = [
    { id: 'pdf', name: 'Reporte PDF', desc: 'Ideal para presentaciones oficiales.', icon: FileText, color: 'text-red-400' },
    { id: 'excel', name: 'Hoja Excel (XLSX)', desc: 'Datos crudos para análisis externo.', icon: Table, color: 'text-green-400' },
    { id: 'json', name: 'Data JSON', desc: 'Para integración con otros sistemas.', icon: Code, color: 'text-blue-400' },
  ];
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div className="bg-white/5 border border-white/5 rounded-[2rem] p-6 relative overflow-hidden group">
                  ...
                  <h4 className="text-[10px] font-black text-yellow-500 uppercase tracking-[0.3em] mb-5">Contenido del archivo</h4>
                  <ul className="space-y-4 relative z-10">
                    {[
                      "Métricas de desempeño docente",
                      "Ranking de las mejores facultades",
                      "Alertas de umbral de rendimiento",
                      "Gráficos de dispersión históricos"
                    ].map((item, i) => (
              ...
              <div className="space-y-3">
                <h4 className="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em] mb-5">Selecciona el formato</h4>
                {formats.map((f) => (
                  <button 
                    key={f.id}
                  ...
                  >
                    <div className={`${f.color} p-2 bg-white/5 rounded-lg group-hover:scale-110 transition-transform`}>
                      <f.icon size={18} />
                    </div>
                    <div>
                      <p className="text-xs font-black text-white uppercase tracking-tight">{f.name}</p>
                      <p className="text-[9px] text-gray-500 font-bold uppercase mt-0.5">{f.desc}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="mt-10 pt-8 border-t border-white/5 flex justify-end">
              <button className="bg-yellow-400 hover:bg-yellow-500 text-black font-black py-4 px-12 rounded-2xl shadow-[0_10px_30px_rgba(250,204,21,0.2)] transition-all active:scale-95 uppercase text-[11px] tracking-[0.2em]">
                Iniciar Generación
              </button>
            </div>
          </motion.div>
