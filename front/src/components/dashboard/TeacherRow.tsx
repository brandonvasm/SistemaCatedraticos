...
export const TeacherRow = ({ teacher, isLast = false }: Props) => {
  const navigate = useNavigate();

  const isTrendUp =
    !teacher.tendencia_mejora.toLowerCase().includes("baja");
  ...
  return (
    <>
        ...
        <div className="flex flex-col">
          ...
          <span className="text-white font-black text-xl">
            {teacher.promedio_general > 0
              ? teacher.promedio_general.toFixed(2)
              : "0.00"}
          </span>
        </div>
      </td>
      ...
      <td
        className={`
          px-6 py-5 w-[120px] text-right
          ${isLast ? "rounded-br-[2rem]" : ""}
        `}
      >
        <button
          onClick={() =>
            navigate(`/docentes/${teacher.teacher_id}`)
          }
          className="
            ...
            group/btn shadow-xl active:scale-95
          "
        >
          <Eye
            size={12}
